...
function checkProperty(node, prop) {
    ...
    if ((boundVars === null || boundVars === void 0 ? void 0 : boundVars[prop]) !== undefined)
        return null;
...
async function applyReplacements(findings) {
    ...
    if (missingVarIds.length > 0) {
        await Promise.allSettled(missingVarIds.map(async (varId) => {
            try {
    ...
    for (const finding of findings) {
        if (!finding.matchedVariableId || finding.replaced)
            continue;
    ...
    for (let bi = 0; bi < nodeIds.length; bi += NODE_BATCH) {
        ...
        const batchNodes = await Promise.allSettled(batchIds.map((id) => figma.getNodeByIdAsync(id)));
